...
def extract_features(imgs, cspace='GRAY', spatial_size=(32, 32),
                     orient=10, pix_per_cell=8, cell_per_block=2, hog_channel = 'ALL'):
...
def convert_color(img, conv='RGB2GRAY'):
    if conv == 'RGB2GRAY':
...
def group_bbox(bbox_list):
    gbbox = []
    return gbbox

def draw_labeled_bboxes_2(img, labels):
    bbox_list = []
    # Iterate through all detected cars
    for car_number in range(1, labels[1]+1):
        # Find pixels with each car_number label value
